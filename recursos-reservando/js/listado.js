...
Listado.prototype.buscarRestaurante = function(id) {
    let restoXid= this.restaurantes.find(cadaRestaurante => cadaRestaurante.id=== id)
    return restoXid ||"No se ha encontrado ningún restaurant";
    } 



filtrarSinRepetir= function(arregloTotal){
    let arregloSinRepetir= arregloTotal.filter(function(elem,index,self){
        return index === self.indexOf(elem)});
        return arregloSinRepetir.sort();
        }
...
Listado.prototype.obtenerCiudades = function() {
    ...
    var todasLasCiudades = this.restaurantes.map((restaurante)=> restaurante.ubicacion);
    //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse
    return filtrarSinRepetir(todasLasCiudades);
}
...
Listado.prototype.obtenerRubros = function() {
    let filtroRestaurantes = this.restaurantes.map((restaurante)=> restaurante.rubro);
    return filtrarSinRepetir(filtroRestaurantes);
   }
...
Listado.prototype.obtenerHorarios = function() {
    ...
    arregloHorarios.forEach(function(a) {
        a.forEach(function(horario) {
            horarios1x1.push(horario)
        });
    });
    return filtrarSinRepetir(horarios1x1);
}
